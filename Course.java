...
public class Course implements Comparable<Course> {
  ...
  public Course(String deptName, int courseNum, int numCredits, int seats) {
    ...
    if(deptName == null||deptName.equals("")||courseNum<= 0||numCredits > 5||numCredits < 1||seats < 0){
  ...
  public void setProfessor(String profName, double rating) throws IllegalArgumentException {
    // TODO: complete this
    if(profName != null){ // only when name isn't null
      if(rating < 0 || rating > 5){
        throw new IllegalArgumentException("Invalid Rating"); // if rating is out of 0-5  range
      }
  ...
  public void setSeatsAvailable(int numSeatsAvailable)throws IllegalArgumentException {
    ...
    if(numSeatsAvailable < 0){
      throw new IllegalArgumentException("Number of seats negative");
    }
  ...
  public int compareTo(Course otherCourse) {
    ...
    if(this.DEPT_NAME.equals("CS") && !otherCourse.equals("CS")){
      return 1; // this course is in major department
    }
    if(!this.DEPT_NAME.equals("CS") && otherCourse.equals("CS")){
      return -1; // this course is not in major department
    }
    if(this.numSeatsAvailable>0 && otherCourse.numSeatsAvailable==0){
      return 1; // only this course has seats available
    }
    if (this.numSeatsAvailable==0 && otherCourse.numSeatsAvailable>0) {
      return -1;// only the other course has seats available
    }
    if(this.profName != null && otherCourse.profName==null){
      return 1; // professor name present
    }
    if(this.profName == null && otherCourse.profName!=null){
      return -1;//Only other professor name present
    }
    if(this.profName != null && otherCourse.profName != null){
      if(this.profRating > otherCourse.profRating){
        return 1; // greater prof rating
      }
      if(this.profRating < otherCourse.profRating){
        return -1; // lesser prof rating
      }
    }
    return 0; //everything is at par
    // TODO complete this method. Be sure to add an @Override annotation as appropriate.
  }
}
